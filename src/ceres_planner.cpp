...
{
  StraightLineCost2D(double l1, double l2, double tx, double ty, double p00, double p01, double p11,
                     double weight)
...
{
  StraightLineCost4D(double l1, double l2, double l3, double anchorX, double anchorY,
                     double anchorC, double anchorS, const std::array<double, 16>& projection,
                     double weight)
    : l1_(l1)
    , l2_(l2)
    , l3_(l3)
...
CeresPlanner::CeresPlanner(std::shared_ptr<RobotMechanism> arm,
                           std::shared_ptr<FCLCollisionChecker> checker, int numWaypoints,
...
{
  if (numWaypoints_ < 2)
    throw std::invalid_argument("At least 2 waypoints required.");
  if (jointCount_ < 2 || jointCount_ > 3)
    throw std::invalid_argument("CeresPlanner currently supports 2 or 3 planar joints.");
  if (options_.lineConstraintWeight <= 0.0)
    throw std::invalid_argument("lineConstraintWeight must be positive.");
  if (options_.smoothnessWeight <= 0.0)
    throw std::invalid_argument("smoothnessWeight must be positive.");
  if (options_.maxNumIterations <= 0)
    throw std::invalid_argument("maxNumIterations must be positive.");
...
void CeresPlanner::setStartGoal(const JointManifoldState& start, const JointManifoldState& goal)
{
  ...
  if (taskSpaceType_ != TaskSpaceType::Euclidean2D && taskSpaceType_ != TaskSpaceType::SE2)
  {
    throw std::invalid_argument(
        "CeresPlanner currently supports 2D Euclidean or SE(2) task spaces.");
  }
}
...
bool CeresPlanner::solve(double solveTimeSeconds)
{
  ...
  if (!computeTaskSpaceDirection(direction))
    return false;
  if (!initializeWaypoints())
    return false;
  if (!optimizePath(direction, solveTimeSeconds))
    return false;
  return validatePath();
}
...
bool CeresPlanner::initializeWaypoints()
{
  ...
  const char* unreachableMessage = isEuclidean2D ? "Straight line passes through unreachable "
                                                   "workspace!" :
                                                   "Straight line passes through unreachable task "
                                                   "space!";
...
bool CeresPlanner::optimizePath(const std::vector<double>& direction, double solveTimeSeconds)
{
  ...
  if (kinParams.linkLengths.size() < requiredJointParams)
    return false;
  ...
  for (int i = 0; i < numWaypoints_; ++i)
  {
    ...
    if (isEuclidean2D)
    {
      lineCost = new ceres::AutoDiffCostFunction<StraightLineCost2D, 2, 2, 2>(
          new StraightLineCost2D(l1, l2, startTaskSpace_[0], startTaskSpace_[1], p00, p01, p11,
                                 options_.lineConstraintWeight));
    }
    else
    {
      lineCost =
          new ceres::AutoDiffCostFunction<StraightLineCost4D, 4, 2, 2, 2>(new StraightLineCost4D(
              l1, l2, l3, startTaskSpace_[0], startTaskSpace_[1], startTaskSpace_[2],
              startTaskSpace_[3], projection, options_.lineConstraintWeight));
    }
  ...
  for (int i = 0; i < numWaypoints_ - 1; ++i)
  {
    for (std::size_t joint = 0; joint < jointCount_; ++joint)
    {
      auto* smoothCost = new ceres::AutoDiffCostFunction<SmoothnessCost, 2, 2, 2>(
          new SmoothnessCost(options_.smoothnessWeight));
      std::vector<double*> smoothBlocks = { jointPathBlock(joint, i),
