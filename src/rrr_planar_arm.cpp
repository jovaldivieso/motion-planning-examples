...
RRRPlanarArm::RRRPlanarArm(double link1Length, double link2Length, double link3Length,
                           double linkThickness, double objectHeight)
...
std::size_t RRRPlanarArm::getJointCount() const
{
  return 3;
}

TaskSpaceCoordinates
RRRPlanarArm::computeTaskSpaceCoordinates(const JointManifoldState& state) const
{
...
{
  if (targetWorkspace.size() < 4 || seedState.size() < 6)
    return false;
  ...
  const JointManifoldState candidateA = { j1A[0], j1A[1], joint2A[0], joint2A[1], c3A, s3A };
  const JointManifoldState candidateB = { j1B[0], j1B[1], joint2B[0], joint2B[1], c3B, s3B };

  if (computeManifoldDistance(candidateA, seedState) <=
      computeManifoldDistance(candidateB, seedState))
  {
...
JointManifoldState RRRPlanarArm::interpolateManifoldState(const JointManifoldState& a,
...
double RRRPlanarArm::computeManifoldDistance(const JointManifoldState& a,
                                             const JointManifoldState& b) const
{
