...
void OMPLPlanner::setOMPLStateFromManifold(const JointManifoldState& state,
                                           ob::State* outState) const
{
...
OMPLPlanner::OMPLPlanner(std::shared_ptr<RobotMechanism> arm) : arm_(std::move(arm))
{
  stateSpace_ = createStateSpace();
  simpleSetup_ = std::make_shared<og::SimpleSetup>(stateSpace_);
}

void OMPLPlanner::setStateValidityChecker(
    const std::function<bool(const JointManifoldState&)>& checker)
{
  simpleSetup_->setStateValidityChecker(
      [this, checker](const ob::State* state) { return checker(manifoldStateFromOMPL(state)); });
}
