...
RRPlanarArm::RRPlanarArm(double link1Length, double link2Length, double linkThickness,
                         double objectHeight)
...
std::size_t RRPlanarArm::getJointCount() const
{
  return 2;
}
...
{
  if (targetWorkspace.size() < 2 || seedState.size() < 4)
    return false;
...
JointManifoldState RRPlanarArm::interpolateManifoldState(const JointManifoldState& a,
...
double RRPlanarArm::computeManifoldDistance(const JointManifoldState& a,
