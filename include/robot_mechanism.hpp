...
public:
  ...
  [[nodiscard]] virtual TaskSpaceCoordinates
  computeTaskSpaceCoordinates(const JointManifoldState& state) const = 0;

  virtual void computeForwardKinematics(const JointManifoldState& state,
                                        std::vector<fcl::Transform3d>& transforms) const = 0;

  virtual void computeEndEffectorPose(const JointManifoldState& state,
                                      fcl::Transform3d& transform) const = 0;

  [[nodiscard]] virtual std::vector<std::shared_ptr<fcl::CollisionGeometryd>>
  getCollisionGeometries() const = 0;
